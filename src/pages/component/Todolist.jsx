  ...
  return (
    <section className="todo-card" aria-label="등반 중 할일 정하기">
      ...
      {showInput && (
        <div className="add-row">
          <input
            ...
            autoFocus
          />
          <button className="add-confirm" onClick={addItem} type="button" aria-label="추가">
            추가
          </button>
        </div>
      )}
      ...
      <ul className="todo-list">
        {items.map((it) => (
          <li key={it.id} className="todo-item">
            ...
            <button
              ...
              title="삭제"
            >
              <svg viewBox="0 0 24 24" className="icon-minus" aria-hidden="true">
        ...
        {items.length === 0 && (
          <li className="todo-empty">아직 할 일이 없어요. 오른쪽 위 <b>+</b> 버튼으로 추가해보세요!</li>
        )}
      </ul>
    </section>
  );
